...
$SOURCE_DIR=$pwd.Path
$TEMP_REPO_DIR="$pwd/../Here-gh-pages"
...
git clone -q https://github.com/KeRNeLith/Here.git --branch gh-pages $TEMP_REPO_DIR
...
if (-not [string]::IsNullOrEmpty($(git status --porcelain)))
{
    ...
    git remote set-url origin https://$($env:GITHUB_ACCESS_TOKEN)@github.com/KeRNeLith/Here.git
